...
public class PrimaryBackupServiceContext implements ServiceContext {
	...
	public PrimaryBackupServiceContext(String serverName, PrimitiveId primitiveId, PrimitiveType primitiveType,
			PrimitiveDescriptor descriptor, ThreadContext threadContext,
			ClusterMembershipService clusterMembershipService, MemberGroupService memberGroupService,
		...
		this.log = ContextualLoggerFactory.getLogger(getClass(), LoggerContext.builder(PrimitiveService.class)
				.addValue(serverName).add("type", descriptor.type()).add("name", descriptor.name()).build());
	 ...
	 *
	 * @param term
	 *            the term to which to reset the current term
	 * @param primary
	 *            the primary for the given term
	 */
	...
	/**
	 * Increments the current index and returns true if the given index is the next
	 * index.
	 *
	 * @param index
	 *            the index to which to increment the current index
	 ...
	 *
	 * @param index
	 *            the index to which to reset the current index
	 * @param timestamp
	 *            the timestamp to which to reset the current timestamp
	 */
			...
			if (term.term() > currentTerm) {
				...
				if (Objects.equals(primary, clusterMembershipService.getLocalMember().id())) {
					if (this.role == null) {
						...
						log.debug("{} transitioning to {}", clusterMembershipService.getLocalMember().id(),
								Role.PRIMARY);

						// if role change from null to primaryRole because startup order ,sync data fro
						// previous primaryRole

						if (term.candidates() != null && !term.candidates().isEmpty()) {
							GroupMember groupMember = term.candidates().get(0);
							requestRestore(groupMember.memberId(), 0);
						}
	...
	/**
	 * sync data from previous primary data
	 * 
	 * @param memberId
	 * @param retryCount
	 */
	private void requestRestore(MemberId memberId, int retryCount) {
		if (this.role instanceof PrimaryRole) {
			this.protocol().restore(memberId, RestoreRequest.request(descriptor(), currentTerm()))
					.whenCompleteAsync((response, error) -> {
						if (error == null && response.status() == PrimaryBackupResponse.Status.OK) {
							resetIndex(response.index(), response.timestamp());
							Buffer buffer = HeapBuffer.wrap(response.data());
							int sessions = buffer.readInt();
							for (int i = 0; i < sessions; i++) {
								getOrCreateSession(buffer.readLong(), MemberId.from(buffer.readString()));
							}
							...
							if (retryCount < 5) {
								...
								requestRestore(memberId, retryCount + 1);
							}
